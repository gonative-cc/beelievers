...
async function main() {
  ...
  program
    .requiredOption(
      "-f, --file <path>",
      "Path to the CSV file with addresses/SuiNS names",
    )
    .option(
      "-b, --batch-size <number>",
      "Number of addresses per transaction",
      "50",
    )
    .option(
      "-o, --offset <number>",
      "Start minting from this entry index (for resuming)",
      "0",
    );
...
async function writeReportCSV(reportEntries: ReportEntry[], filename: string) {
  ...
  const rows = reportEntries
    .map(
      (entry) =>
        `${entry.address},${entry.status},"${entry.reason.replace(/"/g, '""')}"`,
    )
    .join("\n");
