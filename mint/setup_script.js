...
const LOCAL_KIOSK_PKG =
  "0xc25518ca0079c243b7973f5ebe3e8a390c71ab2f2a2cad1dcd11a6066a98ca15";
...
const CONFIGS = {
  testnet: {
    AUCTION_CONTRACT:
      "0x60f6241088efad8dea782dfbf0071aaf58cb1baa60388f4b5e66959f7eec7ef6",
    PACKAGE_ID:
      "0xae0aab765399e42c536b26ee8044efbcdfd4346926a88b26847bf5541cde45ed",
    ADMIN_CAP:
      "0xb71162810cf2683a763bd7b2400e918e8c5d95a7890450bb8d888e84c0cfd253",
    TRANSFER_POLICY_CAP:
      "0x3d35d99f054e3c7ee6fe18c4c8f1fb1900da6d219b8a84b03051cf932d3b56fd",
    TRANSFER_POLICY_ID:
      "0x23039cf016e97829afb865ba1db63e342914c11eca76f87cd4b3d3b4e8e63741",
    COLLECTION_ID:
      "0x565d980198d3bd1ec33107110ccc5ad448dacb19f1495da313cb015971bcd146",
    KIOSK: "0x5f3f8963055c7fac44340e82e88e4b79d7abfa36382b006a8e0b9b803cdfb088",
    KIOSK_CAP:
      "0x81a267691ff67fd44faf14ba4ab773c22ec306907c8b59af8690b8213e8a6efd",
  ...
  local: {
    AUCTION_CONTRACT:
      "0x5f549af160e14603de6102874ff5ec25d8f89c2d137d4a9bfb75977abdab385b",
    PACKAGE_ID:
      "0xd0af67c0281e294200260fc18f2760aa9f8ae7e7a6addd472486154dcf12b50a",
    ADMIN_CAP:
      "0xb30b32bc7a69871eb46647f16997adfed3407f0252052d745f62e62674a04b6a",
    TRANSFER_POLICY_CAP:
      "0x36f9da986f880c01ee2530c475ce005828fc517792c88d5573d950d4fce0bd58",
    TRANSFER_POLICY_ID:
      "0x64b8df7d67c5ae69f8c4dfe82401361341a936ed2a2c90be24b5e9f60ae39b8d",
    COLLECTION_ID:
      "0x567076a52f4952b1f93fafc1f116c1c82c54c945bef2db84830a1ce79c5d867b",
    KIOSK: "0xa20173e280e076af4c8a293c6a03569ea2a64bdad571daae9c9d341317171360",
    KIOSK_CAP:
      "0xf353f31e1a78d02ea1322cb3e744c4faa52186673ee328bd6cd7ffdd70f0437a",
  ...
  production: {
    ...
    PACKAGE_ID:
      "0x3aeca4699ce5f914b56ee04b8ccd4b2eba1b93cabbab9f1a997735c52ef76253",
    ADMIN_CAP:
      "0x9e0e0b8c9b68465caaf1e28cfff2c05f7a135503977995670884f57fc8b8ceb6",
    TRANSFER_POLICY_CAP:
      "0xe83a713d437607737229a4ae32c056b7893417c14dc399c4696f74a238e4a9a7",
    TRANSFER_POLICY_ID:
      "0xd9fe40ec079a6959940260c29be5a782cb79a7951906a0ac380a3961dbd78914",
    COLLECTION_ID:
      "0xe896f82d681a0562a3062fff61a72c3ac324be5a4f00fa6db0f9520a4124ce7b",
    KIOSK: "0x6384ef33995306a29c4e979aee39bec4b04d4094aaa1a6062d995f985dd9b343",
    KIOSK_CAP:
      "0x5410c13fd6156a8baa44091ee6682129ffded0dacbcca7944c339aa444208a7f",
...
function getClientAndSigner() {
  ...
  const signer = Ed25519Keypair.fromSecretKey(
    suiprivkeyToHex(ADMIN_PRIVATE_KEY),
  );
...
async function addProductionMythicEligible() {
  ...
  try {
    ...
    if (mythicEligible.length === 0) {
      throw new Error(
        "No mythic eligible addresses found in mythic_eligible.txt",
      );
    }

    log(
      `Found ${mythicEligible.length} mythic eligible addresses to add`,
      "INFO",
    );
    ...
    for (
      let i = 0;
      i < mythicEligible.length;
      i += MYTHIC_ELIGIBLE_BATCH_SIZE
    ) {
      const batchEnd = Math.min(
        i + MYTHIC_ELIGIBLE_BATCH_SIZE,
        mythicEligible.length,
      );
...
async function setBadgeNamesFromJson() {
  ...
  try {
    ...
    const badgeNamesData = JSON.parse(
      await fs.readFile("badge_names.json", "utf8"),
    );
    ...
    log(
      `Error reading or processing badge_names.json: ${error.message}`,
      "ERROR",
    );
    throw error;
  }
...
async function setMinterBadgesFromJson() {
  ...
  try {
    ...
    if (!badgeData || Object.keys(badgeData).length === 0) {
      log(
        "No badge data found in badges.json, skipping minter badge assignment",
        "WARNING",
      );
      return;
    }
    ...
    log(
      `Found ${Object.keys(badgeData).length} address entries with badge data`,
      "INFO",
    );
    ...
    const validEntries = addresses.filter(
      (_, index) => badgeNumbers[index].length > 0,
    );
    const validBadgeNumbers = badgeNumbers.filter(
    ...
    for (let i = 1; i <= maxNfts; i += config.BATCH_SIZE) {
      ...
      for (let j = i; j <= batchEnd; j++) {
        ...
        try {
          ...
          log(
            `Error reading attributes for NFT #${j}: ${error.message}`,
            "WARNING",
          );
          continue;
        }
      ...
      if (batchNftIds.length > 0) {
        ...
        try {
          ...
          log(
            `Batch: processed attributes for ${batchNftIds.length} NFTs`,
            "SUCCESS",
          );
...
async function setUrlsFromJson() {
  ...
  try {
    ...
    const imageLinksData = JSON.parse(
      await fs.readFile("imagelinks.json", "utf8"),
    );
    const processedUrls = new Set();

    log(
      `Found ${Object.keys(imageLinksData).length} URL entries in imagelinks.json`,
      "INFO",
    );

    const maxNfts =
      IS_TEST && config.TEST_URLS_LIMIT
        ? config.TEST_URLS_LIMIT
        : config.TOTAL_NFTS;
    ...
    for (let i = 1; i <= maxNfts; i += config.BATCH_SIZE) {
      ...
      if (nftIds.length > 0) {
        ...
        try {
          ...
          log(
            `Error processing batch ${i}-${batchEnd}: ${error.message}`,
            "ERROR",
          );
          throw error;
        }
    ...
    log(
      `Completed processing URLs. Total NFTs processed: ${processedUrls.size}`,
      "SUCCESS",
    );
  } catch (error) {
    log(
      `Error reading or processing imagelinks.json: ${error.message}`,
      "ERROR",
    );
    throw error;
  }
...
async function enableMinting() {
  ...
  log(
    `Minting will start at ${new Date(startTime).toISOString()} (${startTime})`,
    "INFO",
  );
...
async function createKiosk() {
  ...
  console.log(
    "Kiosk creation successful, tx id:",
    result.digest,
    "\nEffects.created:",
  );
  ...
  txEffects.effects.created.forEach((obj) => {
    if (obj.owner.Shared) {
      ...
      obj.owner.AddressOwner === signer.getPublicKey().toSuiAddress()
    ) {
